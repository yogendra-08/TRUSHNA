    ...
    if (options?.wakeWord) {
      if (!isAwakeRef.current) {
        if (finalText.includes(options.wakeWord.toLowerCase())) {
          ...
          const commandAfterWakeWord = finalText.substring(
            finalText.indexOf(options.wakeWord.toLowerCase()) + options.wakeWord.length
          ).trim();
          ...
          if (commandAfterWakeWord) {
            ...
            Promise.resolve().then(() => {
              if (isAwakeRef.current && recognitionRef.current) {
                setIsListening(true);
